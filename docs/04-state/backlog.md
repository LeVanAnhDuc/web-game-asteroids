...
Không có việc nào đang dở. Feature `asteroids-core` đã xong toàn bộ 10 task của `docs/specs/asteroids-core/plan.md` trên branch `feat/asteroids-core`: 151 test xanh, `typecheck`/`lint`/`build` sạch, bundle 117 kB. Việc còn lại đều là việc mới, nằm ở mục dưới.
...
| Xem tận mắt ba khổ 375 / 768 / 1024                       | FR-17 · NFR-A11Y-03 | cao        | T9 chỉ kiểm được ở khổ rộng: trong tab automation, lệnh đổi kích thước cửa sổ không đổi được viewport và `requestAnimationFrame` bị throttle. Logic đã có test, nhưng bố cục thì phải nhìn |
